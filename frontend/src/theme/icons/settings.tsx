...
function SettingsSvg({ strokeWidth }: { strokeWidth: number }) {
  return (
...
export function SettingsNavIcon({ deviceTheme }: Props) {
  const style = DEVICE_THEMES[deviceTheme].iconStyle;
  if (style === "outline") return <SettingsSvg strokeWidth={1.5} />;
  if (style === "bold") return <SettingsSvg strokeWidth={3.5} />;
